...
public class P126
{
    public static void main(String[] args)
    {
        ...
        System.out.println(new Solution().findLadders("hit", "cog", Arrays.asList("hot","cog","dot","dog","hit","lot","log")));
    ...
    public static class Solution {
        public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList)
        {
            ...
            String[] ss = new String[wordList.size() + 2];
            int wc = 0;
            ss[wc++] = beginWord;
            if (!wordList.contains(endWord)) return res;
            for (String s : wordList) {
                if (s.equals(beginWord)) continue;
                if (s.equals(endWord)) continue;
                ss[wc++] = s;
            }
            ss[wc++] = endWord;

            int[][] dp = new int[wc][wc];

            for (int i = 0; i < wc; ++i) {
                for (int j = 0; j < wc; ++j) {
                    if (hasEdge(ss[i], ss[j])) {
                        dp[i][j] = 1;
                    } else {
                        dp[i][j] = 1000;
                    }
            ...
            boolean[] map = new boolean[ss.length];
            Arrays.fill(map, false);
            map[0] = true;

            int[] level = new int[ss.length];
            Arrays.fill(level, 0);
            level[0] = 1;
            ...
            Arrays.fill(map, false);
            map[wc - 1] = true;
            findPath3(ss, dp, wc - 1, pathLen - 1, stack, res, map, level, beginWord);

            return res;
        }

        private void findPath3(String[] ss, int[][] dp, int idx, int len, Stack<String> stack,
                               List<List<String>> res, boolean[] map, int[] level, String beginWord)
        {
            if (len <= 0) return;
            if (len == 1) {
                for (int i = 0; i < dp.length; ++i) {
                    if (dp[i][idx] == 1 && level[i] == 1 && !map[i] && ss[i].equals(beginWord)) {
                        List<String> tlist = new LinkedList<>(stack);
                        tlist.add(beginWord);
            ...
            for (int i = 0; i < dp.length; ++i) {
                if (dp[i][idx] == 1 && level[i] == len && !map[i]) {
                    stack.push(ss[i]);
                    map[i] = true;
                    findPath3(ss, dp, i, len - 1, stack, res, map, level, beginWord);
                    stack.pop();
                    map[i] = false;
                }
            }
        ...
        private void findPath2(String[] ss, int idx, int[][] dp, int len, Stack<String> stack,
                              List<List<String>> res, boolean[] map, int[] level, String endWord)
        {
            if (stack.size() >= len) return;
            if (stack.size() == len - 1) {
                for (int i = 0; i < dp.length; ++i) {
                    if (dp[idx][i] == 1 && level[i] == stack.size() + 1 && !map[i] && ss[i].equals(endWord)) {
                        stack.push(ss[i]);
            ...
            for (int i = 0; i < dp.length; ++i) {
                if (dp[idx][i] == 1 && level[i] == stack.size() + 1 && !map[i]) {
                    stack.push(ss[i]);
                    map[i] = true;
                    findPath2(ss, i, dp, len, stack, res, map, level, endWord);
                    stack.pop();
                    map[i] = false;
                }
            }
        ...
        public List<List<String>> findLadders1(String beginWord, String endWord, List<String> wordList)
        {
            ...
            Arrays.fill(map, false);
            map[0] = true;
            findPath(ss, 0, dp, dp[0][wc - 1], stack, res, map);
            return res;
        }

        private void findPath(String[] ss, int idx, int[][] dp, int len, Stack<String> stack,
                              List<List<String>> res, boolean[] map)
        {
            if (stack.size() == len + 1) {
                res.add(new LinkedList<>(stack));
                return;
            ...
            for (int i = 0; i < dp.length; ++i) {
                if (dp[idx][i] == 1 && dp[i][dp.length - 1] == len - stack.size() && !map[i]) {
                    stack.push(ss[i]);
                    map[i] = true;
                    findPath(ss, i, dp, len, stack, res, map);
                    stack.pop();
                    map[i] = false;
                }
            }
